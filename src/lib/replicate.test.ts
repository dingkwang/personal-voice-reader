...
it("preflights regeneration against used attempts and queued reservations without increasing the cap", async () => {
  ...
  const queued = await createReading({ text: "另一篇待生成合成测试", voice_id: "voice_replicate", speed: 1, idempotency_key: "queued-reservation" });
  ...
  await expect(regenerateDocument(source.documentId, payload, TEST_OWNER)).rejects.toMatchObject({ code: "QUOTA_EXCEEDED" });
  expect((await fixture.db.query("SELECT * FROM replicate_requests")).rows).toHaveLength(100);
});
it("whole regeneration makes one new provider submission per item, retains old audio, and retries idempotently", async () => {
  ...
  const source = await createReading({ text: "合成第一段。\n\n合成第二段。\n\n合成第三段。", voice_id: "voice_replicate",
    speed: 1, idempotency_key: "replicate-all-source" });
  ...
  expect((await readingStatus(next.jobId)).status).toBe("completed");
});
