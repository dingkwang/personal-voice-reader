...
export async function addVoice(voice: Voice, owner: string, db = database()) {
...
export async function findDocument(id: string, owner: string, db = database()): Promise<ReaderDocument> {
...
export async function findSegment(id: string, owner: string, db = database()) {
...
export async function listDocuments(owner: string): Promise<ReaderDocument[]> {
  const db = database();
  const [{ rows }, segments] = await Promise.all([
...
export async function addDocument(document: ReaderDocument, owner: string) {
...
export async function readStore(owner: string): Promise<StoreData> {
